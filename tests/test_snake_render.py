...
class TestSnakeRender(unittest.TestCase):
    def test_render_stack_survives_snake_style_cli_animation(self):
        ...
        self.assertTrue(any(f"snake tick {frame_count}" in line for line in lines))
        self.assertGreaterEqual(len(lines), board.render_height * 100)
        self.assertTrue(all(len(line) == columns for line in final_frame))
        self.assertTrue(all(visible_length(line) == columns for line in board.render_lines()))
